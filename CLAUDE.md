...
| `bun run dev` | `phy-simulator run . --type screen --dev-command 'bun run start'` — twin on the running simulator + vite dev server, auto-opens the browser (`--type screen`: the simulator has no native `web` type yet; web twins are structurally screen twins) |
